...
A hands-free voice interface for Codex. The goal: talk to Codex through Bluetooth earbuds (Pixel Buds) on an Android phone while your hands are busy — like working at a bakery — and have Codex working on code, managing repos, and talking back.

## Origin

This project was spun off from the `evanstern/ideas` repo after exploring the concept. The core insight: Codex's web app (Codex.ai/code) works today with voice typing, but what we really want is a **full voice loop** — continuous speech-to-text input, Codex processing with tool use, and text-to-speech output back through earbuds.

## Architecture

```
Pixel Buds → Phone Mic → Speech-to-Text (Whisper / Google STT)
    → Codex API (with tool_use for git, file ops, bash)
    → Text-to-Speech (OpenAI TTS / ElevenLabs / Google TTS)
    → Pixel Buds speaker
```

**Components:**
- **Server (`apps/server`)** — Hono + tRPC backend running on a home Mac. Manages the Codex API session, executes tools (git, file I/O, shell commands), handles the STT→Codex→TTS pipeline. WebSocket endpoint for real-time audio streaming.
...
- **UI Library:** `@voice-Codex/ui` (shared components with CVA + tailwind-merge)
- **Contracts:** `@voice-Codex/contracts` (shared Zod schemas)
- **API:** Anthropic Codex API with tool_use
...
- **Tool execution context** — Codex needs access to repos, files, and shell on the server. Security model matters.
...
4. Connect to Codex API with a simple tool (e.g., read a file)
5. Wire up TTS on Codex's response
6. Test the full loop: speak → transcribe → Codex → synthesize → hear
...
| `ANTHROPIC_API_KEY` | -- | Required for Codex API |
| `OPENAI_API_KEY` | -- | Required for Whisper STT / TTS |
| `WORK_DIR` | `/workspace` | Directory mounted into server for Codex tool execution |

### Volume Mounts
The server container mounts `WORK_DIR` (defaults to `./workspace`) into `/workspace` inside the container. This is the directory Codex tools operate on (git, file ops, shell).
